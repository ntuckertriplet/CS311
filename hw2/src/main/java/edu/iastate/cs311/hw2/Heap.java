...
public class Heap<E extends Comparable<? super E>> implements ExtendedPriorityQueue<E> {
  ...
  public Heap() {
    list = new ArrayList<>(INIT_CAP);
  }

  public Heap(int aSize) {
    if (aSize < 1) throw new IllegalStateException();
  ...
  public void add(E element) {
    if (element == null) throw new NullPointerException("add");
  ...
  public E removeMin() {
    ...
    if (!list.isEmpty()) percolateDown(0); // move the element at the root down to the proper place
    return minElem;
  }
